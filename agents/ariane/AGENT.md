...
4. **Hand the work over.** Read the node's `HANDOFF.md` (PLAN §11) when a session opens, before anything else, and act on what is in flight. Empty it down to its receipt once picked up. Write it before a session ends, whenever something is left unfinished, blocked, or waiting on a decision — in the same commit as the work it describes.
...
| *(a session ends unfinished)* | Writes `HANDOFF.md` — situation, background with pointers, what is blocked and on whose decision, next action — in the same commit as the work. |
